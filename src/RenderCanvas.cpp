...
RenderCanvas::~RenderCanvas() { delete[] this->pixels; }

void RenderCanvas::setPixel(int x, int y, int_color color) {
  // We use height - 1 - y because the origin of the image is at the bottom left
  // corner
  const int index =
      (x + (this->height - 1 - y) * this->width) * CANVAS_CHANNELS;
...
int_color RenderCanvas::getPixel(int x, int y) {
  ...
  return calculateColorFromRGB(this->pixels[index + 0], this->pixels[index + 1],
                               this->pixels[index + 2]);
}

void writeImageToDisk(RenderCanvas *canvas, const char *filename) {
  stbi_write_png(filename, canvas->getWidth(), canvas->getHeight(),
                 CANVAS_CHANNELS, canvas->getPixels(), 0);
}
