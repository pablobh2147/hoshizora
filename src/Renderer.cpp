...
void Renderer::render(const Scene& scene, const Camera& camera,
    ...
    /*for (int x = 0; x < canvas->getWidth(); x++) {
            for (int y = 0; y < canvas->getHeight(); y++) {
                    ...
                    Color accum_2 = accumulated_colors[x * 2 + (y * 2 + 1) *
    width]; accum_2 /= accumulated_samples; Color accum_3 = accumulated_colors[x *
    2 + 1 + (y * 2 + 1) * width]; accum_3 /= accumulated_samples;

                    Color accum = (accum_0 + accum_1 + accum_2 + accum_3) / 4.0f;
                    accum = glm::clamp(accum, 0.0f, 1.0f);

                    canvas->setPixel(x, y, calculateColorFromRGBF(accum.r,
    accum.g, accum.b));
            }
    }*/
...
Color Renderer::perPixel(int x, int y) {
    ...
    for (int i = 0; i < bounces; i++) {
        ...
        const Material& material =
            current_scene->materials[payload.sphere->material_idx];
